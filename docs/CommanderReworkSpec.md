...
Status: **Gated / not implemented.** The Commander is unavailable to select until every gate in
[§17 Definition of Done](#17-definition-of-done--the-gate) passes. This document is a design spec, not an
implementation. It defines observable, testable player-facing behavior; exact values are handed to
systems-balance and implementation structure to engineering.

## 0. Why this exists (evidence)

Verified against current source (read-only):

- The Commander has **no functional ability**. Its concept scripts are fully commented out
  (`Assets/Scripts/Abilities/Reroute.cs`, `Assets/Scripts/Abilities/ActivateAbility.cs`), the
  `Commander.prefab` carries no `Ability`-derived component, and `Commander.asset` still advertises a
...
- The Commander is nonetheless **fully selectable**: `CharacterSelectionUIController.BuildRosterOptions`
  lists every entry of `AllUnits.asset`, where the Commander is **index 0**
  (`Commander.asset.meta` guid `c0d4e931…`). `SelectUnit` performs no distinctness check and
  `ConfirmSelectionServerRpc` validates only index range and count, so duplicates and the broken
  Commander are both accepted.
...
Converged recommendation from the completed gameplay and systems-balance audits: **gate the Commander
now, and rework it as a plan-committed "Smoke Screen" support ability.** Both audits converge on Smoke
...
- **Punish the commitment:** the Commander must expose itself to place the smoke and is fragile
  (low HP/damage), so a committed smoke is a readable tell the opponent can punish next round.
...
- **v1 scope: Smoke blocks shot line-of-sight only, not fog vision.** Whether a unit *can be shot*
  through smoke changes; whether a unit *is revealed* by fog does not. This keeps behavior readable and
  avoids new interactions with `UpdateUnitVisibilityForClient`.
- **Stretch (experiment, not v1): vision occlusion.** Optionally also block fog vision through the smoke
  so units behind it are hidden. This is deferred to an instrumented experiment because it adds
  readability and performance risk to the per-client fog update; it must not ship in v1.
...
Shooting and damage already gate on a line-of-sight raycast against the **`Walls`** layer:

- `Shooting.lineOfSight` → `Physics.Raycast(..., LayerMask.GetMask("Walls", enemyTeam))`.
...
2. **Scoring:** value ≈ expected ally damage/deaths prevented (weight high-lethality lanes: Sniper
   `Area Lock` 160, `Grenade` 80) **minus** value denied to the bot's own shots through the same cloud
...
- Whether Smoke also occludes fog vision (stretch experiment; default **off** in v1).
- Commander base stats: keep the light pistol (`damage: 12`) or reduce/remove weapon output in exchange
  for utility; HP; vision. Currently `maxHealth: 100`, `visionRange: 6`.
...
- v1 blocks **shot line-of-sight only**; fog-vision occlusion is an explicit stretch experiment (§7).
